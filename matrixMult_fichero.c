...
int main(int argc, char *argv[])
{
    ...
    char *nombre_ficheroA = argv[2]; //Nombre del archivo a leer
...
void leer_a_matriz(char *nombre_fichero, int *matriz, int N)
{
    char *buff_linea = malloc(N * sizeof(char));

    FILE *fp_a = fopen(nombre_fichero, "r");

    char *pch;
    int i = 0;

    while (fgets(buff_linea, N, fp_a))
    {
        pch = strtok(buff_linea, " ");
